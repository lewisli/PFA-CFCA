...
%  plot velocities with knots at each ageo
...
lhdl = line(agefine, mean(hgtfmat2'));
set(lhdl, 'LineStyle', '--')
xlabel('\fontsize{19} Age')
...
wtvec  = ones(nage,1);  %  weight vector
...
[Wfdobj, C] =  densityfd(resfvec, WfdPar);
...
for iage=1:nage
    disp(iage)
    [Wfdi, C] = densityfd(resftrim(iage,:)', WfdPar, ...
                          1e-3, 10, 2:nWbasis, 0);
    denssave(:,iage) = eval_pos(resfine, Wfdi)./C;
end
 
h = surf(age, resfine, denssave)
