function [bfdcell, afdcell, resfdcell] = ...
    pdacell(xfdcell, bwtcell, awtcell, ufdcell, norder, nfine)
...
if nvar == 1
    ...
    if any(size(bwtcell) ~= [nvar, norder])
    ...
    xrange     = getbasisrange(getbasis(xfdcell{1}));
    ...
    if nforce > 0
        for ivar1=1:nvar
            ...
            for k=1:nforce
                ...
                if ivar1 == 1 & k == 1
                    afdnames = getnames(ufdcell{ivar1,k});
                    %  check that URANGE is equal to XRANGE
